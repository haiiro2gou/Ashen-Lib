#> ashen_lib:array/core/mask
# @within function ashen_lib:array/**mask
...
# マスクデータに合わせて値を追加する
    execute if data storage ashen_lib:api {Mask: true} run data modify storage ashen_lib:api Array append from storage ashen_lib:api MaskElement
    execute if data storage ashen_lib:api {Mask:false} run data modify storage ashen_lib:api Array append from storage ashen_lib:api CopiedArray[-1]
# 末尾削除
