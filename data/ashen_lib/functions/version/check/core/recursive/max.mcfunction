#> ashen_lib:version/check/core/recursive/max
# @within function
...
# Check
    ...
    execute store result score #Comp Ashen.Global.Temporary run data get storage ashen_lib:temp version.max[0]
...
# Loop
    ...
    data remove storage ashen_lib:temp version.max[0]
    execute if score #CheckVersion Ashen.Global.Temporary matches 1 if data storage ashen_lib:temp version.target[0] run function ashen_lib:version/check/core/recursive/max
